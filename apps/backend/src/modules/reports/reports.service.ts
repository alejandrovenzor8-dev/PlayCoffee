...
export class ReportsService {
  ...
  async getSalesSummary(branchId: string, from: string, to: string) {
    ...
    const totalItems = orders.reduce(
      (s, o) => s + o.items.reduce((i, it) => i + it.quantity, 0),
      0,
    );
    ...
    for (const order of orders) {
      for (const payment of order.payments) {
        ...
        byPaymentMethod[method] =
          (byPaymentMethod[method] ?? 0) + Number(payment.amount);
      }
    }
  ...
  async getKpis(branchId: string) {
    ...
    const [todayOrders, monthOrders, activeTables, activeChildren] =
      await Promise.all([
        ...
        this.prisma.childAccess.count({
          where: { branchId, exitTime: null, type: 'ENTRY' },
        }),
      ]);
  ...
  async getSummary(branchId?: string, from?: string, to?: string) {
    ...
    for (const order of orders) {
      const day = order.createdAt.toLocaleDateString('es-MX', {
        weekday: 'short',
      });
    ...
    const salesByDay = Array.from(salesMap.entries()).map(([date, v]) => ({
      date,
      ...v,
    }));
    ...
    const topProduct =
      [...productMap.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? '—';
